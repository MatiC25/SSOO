 ...
 int crear_servidores(t_config_cpu* config_cpu, int* md_cpu_ds, int* md_cpu_it) 
{
    ...
    *md_cpu_ds = iniciar_servidor("DISPATCH", NULL, puerto_dispatch); // Guarda ID del socket
...
void iniciar_modulo(t_config_cpu* config_cpu) {
    ...
    if(crear_servidores(config_cpu, &md_cpu_ds, &md_cpu_it) != 1) {
    ...
    t_procesar_conexion* args_ds = crear_procesar_conexion("DISPATCH", md_cpu_ds);
    t_procesar_conexion* args_it = crear_procesar_conexion("INTERRUPT", md_cpu_it);
