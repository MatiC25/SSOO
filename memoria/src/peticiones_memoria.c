...
void* escuchar_peticiones(void* args){
    ...
    while (1)
    {
        ...
        {
        case HANDSHAKE:
            generar_handshake(socket_cliente, server_name, NULL ,string_itoa(config_memoria->puerto_escucha));
            break;
...
void handshake_desde_memoria(int socket_cliente){
	...
	agregar_a_paquete(paquete, &config_memoria->tam_pagina, sizeof(config_memoria->tam_pagina));
